...
# COF: These dont add information
...
n_folds <- 2
docv <- function(param0, iter, n_folds) {
  model_cv = xgb.cv(
    params = param0
    , nrounds = iter
    , nfold = n_folds
...
rn_val <- runif(1 , min = 1e4, max = 1e6); rn_val
set.seed( rn_val )
cv <- docv(param0, 1500, n_folds) 
# Show the clock
print( difftime( Sys.time(), start_time, units = 'min'))
...
for (i in 1:n_ens) {
  print(i)
  rn_for <- runif(1 , min = 1e4, max = 1e6) ; rn_for
  set.seed( rn_for )
  p <- doTest(param0, cv) 
...
file_out <- paste("Res_XXXX_XGB_cv_", n_folds, "_plus_", pl_us, "_ens_", n_ens,"_time_", dat_tim, "_.csv", sep = "" )
write.csv(submission, file = file_out, row.names = F, quote = F)
