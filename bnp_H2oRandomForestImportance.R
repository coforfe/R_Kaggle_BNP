...
res_df <- data.frame(rfAcc = 0, mx_dp = 0, sa_ra = 0,
                     ma_tr = 0, ex_t = 0)
...
file_tmp <- paste("Res_xxxxx_H2O_rf_grid_",nrow(rfGrid),"_", round(Accrf,5),"_",dat_tim,"_.csv", sep = "")
write.csv(submission,file_tmp,row.names = F)

} #for( i in 1:nco
file_wt <- paste("res_df_rf_grid_",nrow(rfGrid),"_",dat_tim,"_.csv", sep = "")
...
library(lattice)
xy_gr <- xyplot(
   #rfAcc ~ max_depth | as.factor(samp_rate) * as.factor(mtries)
   rfAcc ~ max_depth | as.factor(samp_rate) * as.factor(mtries)
  ,data = res_df
  ,type = "b"
